...
export default defineSchema({
  ...
  companyMemberships: defineTable({ companyId: v.id("companies"), userId: v.id("appUsers"), role, active: v.boolean(), firstName: v.optional(v.string()), secondName: v.optional(v.string()), createdAt: v.number(), updatedAt: v.number() }).index("by_company", ["companyId"]).index("by_user", ["userId"]).index("by_company_user", ["companyId", "userId"]),
  ...
  userBranchAssignments: defineTable({ companyId: v.id("companies"), membershipId: v.id("companyMemberships"), branchId: v.id("branches") }).index("by_membership", ["membershipId"]).index("by_branch", ["branchId"]).index("by_membershipId_and_branchId", ["membershipId", "branchId"]),
  userDepartmentAssignments: defineTable({ companyId: v.id("companies"), membershipId: v.id("companyMemberships"), departmentId: v.id("departments") }).index("by_membership", ["membershipId"]).index("by_department", ["departmentId"]).index("by_membershipId_and_departmentId", ["membershipId", "departmentId"]),
  ...
  }).index("by_company", ["companyId"]).index("by_token", ["token"]).index("by_email", ["email"]).index("by_companyId_and_email_and_status", ["companyId", "email", "status"]),
  ...
  jdTaskCompletions: defineTable({ companyId: v.id("companies"), jdTaskId: v.id("jdTasks"), cycleStart: v.number(), cycleEnd: v.optional(v.number()), completedByMembershipId: v.optional(v.id("companyMemberships")), completedAt: v.number(), note: v.optional(v.string()) }).index("by_task", ["jdTaskId"]).index("by_task_and_cycleStart", ["jdTaskId", "cycleStart"]).index("by_task_and_completedAt", ["jdTaskId", "completedAt"]),
  // Kept optional while legacy schedule-generation records remain live.
  jdTaskCycleRecords: defineTable({ companyId: v.id("companies"), jdTaskId: v.id("jdTasks"), cycleStart: v.number(), cycleEnd: v.number(), status: v.literal("missed"), recordedAt: v.number(), scheduleGeneration: v.optional(v.number()) }).index("by_company", ["companyId"]).index("by_task", ["jdTaskId"]).index("by_task_and_cycleStart", ["jdTaskId", "cycleStart"]).index("by_task_and_cycleEnd", ["jdTaskId", "cycleEnd"]),
  ...
  sops: defineTable({ companyId: v.id("companies"), reference: v.string(), title: v.string(), content: v.string(), scopeType: scope, creatorMembershipId: v.id("companyMemberships"), updatedByMembershipId: v.id("companyMemberships"), createdAt: v.number(), updatedAt: v.number() }).index("by_company", ["companyId"]),
  ...
  taskImportBatches: defineTable({ companyId: v.id("companies"), actorMembershipId: v.id("companyMemberships"), kind: taskType, importKey: v.string(), batchKey: v.string(), source: v.union(v.literal("cendro"), v.literal("ai")), requestFingerprint: v.string(), result: v.object({ created: v.number(), updated: v.number(), skipped: v.number(), failed: v.number(), taskReferences: v.array(v.string()) }), createdAt: v.number() }).index("by_companyId", ["companyId"]).index("by_companyId_and_importKey", ["companyId", "importKey"]).index("by_companyId_and_importKey_and_batchKey", ["companyId", "importKey", "batchKey"]),
});
