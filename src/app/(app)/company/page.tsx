    ...
    <div className="app-page">
      ...
      <Tabs defaultValue="branches">
        ...
        <TabsContent value="invitations" className="mt-4">
          <Card className="p-3">
            <div className="mb-3 flex gap-2"><Input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="employee@example.com" /><Button disabled={!email.trim()} onClick={async () => { if (activeCompanyId && email.trim()) { const optimistic = { _id: crypto.randomUUID(), email, role: "Employee", status: "pending" }; setInviteError(null); setPendingInvites((current) => [optimistic, ...current]); try { await invite({ companyId: activeCompanyId, email, role: "Employee" }); setEmail(""); } catch (err) { setInviteError(err instanceof Error ? err.message : "Could not send invitation."); } finally { setPendingInvites((current) => current.filter((inviteRow) => inviteRow._id !== optimistic._id)); } } }}>Invite employee</Button></div>
