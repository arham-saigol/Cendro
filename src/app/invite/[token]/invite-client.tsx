...
export function InviteClient({ token }: { token: string }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--chrome)] p-6">
      <Card className="w-full max-w-md p-6 shadow-[var(--shadow-elevated)]">
        <div className="grid h-10 w-10 place-items-center rounded-md bg-[var(--surface-muted)] text-[var(--ink-muted)]"><Mail className="h-5 w-5" /></div>
        <h1 className="mt-3 text-2xl font-semibold">Cendro invitation</h1>
        {preview === undefined ? <div className="mt-3 animate-pulse space-y-2">
        ...
        {authUnavailable && <p className="alert-error mt-3 rounded-md p-2 text-sm">Could not authenticate your session with Convex. Please refresh and try again.</p>}
