import { z } from "zod";
const serverSchema=z.object({CLERK_SECRET_KEY:z.string().min(1),PLATFORM_ADMIN_EMAIL:z.string().email(),NEXT_PUBLIC_CONVEX_URL:z.string().url(),AI_GATEWAY_API_KEY:z.string().min(1),AI_MODEL:z.string().default("deepseek/deepseek-v4-pro"),FIRECRAWL_API_KEY:z.string().optional()});
